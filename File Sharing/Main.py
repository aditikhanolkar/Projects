...
class MainPage:
    ...
    def __init__(self):
        ...
        self.setupUI()
        self.c = None
        self.imageFileTypes = ("Image Files", ('*.jpg', '*.jpeg', '*.JPG', '*.png', '*.gif', '*.bmp'))
    ...
    def connectedPage(self):
        ...
        self.connectedFrame.pack(fill = BOTH, expand = False)
        ...
        seperator.pack(side = TOP, fill = X)
        ...
        self.sendFileButton.pack(side = BOTTOM)
        
        #self.recvLabel = Label(self.connectedFrame, anchor = 'nw', bg = '#5d5d5d', fg = 'white')
        #self.recvLabel.place(x = 10, y = 60, width = 280, height = 320)
		
        self.recvList = Listbox(self.connectedFrame, bg = '#5d5d5d', fg = 'white')
        self.recvList.pack(side = LEFT, expand = 1, fill = BOTH,  padx = 10, pady = 10, ipady = 100, ipadx = 70)
		
        scrollbar = Scrollbar(self.connectedFrame, orient = "vertical", bd = 0)
        scrollbar.config(command = self.recvList.yview)
        scrollbar.pack(side = RIGHT, fill = Y)
		
        self.recvList.config(yscrollcommand = scrollbar.set)
    ...
    def sendFiles(self, *s):
        ...
        for i in range(len(s)):
            try:
                sock = s[i]
                size = int(os.path.getsize(filelist[0]))
                ...
                while totalsend!=size: 
                    ...
                    totalsend += len(l)
                f.close()
                self.recvList.insert(END, 'Sent - ' + filename[-1])
            except:
                pass
            ...
            try:
                ...
                while True:
                    ...
                    while totalrecv!=size:
                        ...
                        f.write(l)
                    f.close()
                    self.recvList.insert(END, 'Received - ' + name)
            except:
                pass
